...
export default function SocketWrapper({ children }) {
    ...
    useEffect(() => {
        function removeExternalUser() {
            navigate("/", { replace: true })
            toast.error("No username provided")
        }

        location.state && location.state.username ? socket.emit("when a user joins", { roomId, username: location.state.username }) : removeExternalUser()
